use leptos_spin::{render_best_match_to_stream_with_context, RouteTable, server_fn::register_explicit};
...
async fn handle_spin_test(req: IncomingRequest, resp_out: ResponseOutparam) {
    ...
    conf.leptos_options.output_name = "spin_test".to_owned();
    ...
    render_best_match_to_stream_with_context(req, resp_out, &routes, app_router,move ||{provide_context(con.clone()); }, &conf.leptos_options).await
}
